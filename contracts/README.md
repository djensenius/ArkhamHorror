...
`question-player-window-enemy-actions.json`, generated by creating a real
Ghoul Minion engaged with Roland on the deterministic post-setup board and
running the ordinary production `PlayerWindow` action enumeration. The
resulting `PlayerWindowChooseOne` preserves the complete authoritative order:
gain a resource, draw a card, end the turn, investigate, fight, then evade.

The Fight and Evade entries are real `AbilityLabel` values sourced from that
enemy. Backend tests bind both to card code `c01160`, the same enemy source,
their production ability indices (`100` and `101`), and the exact source-array
indices (`4` and `5`). Current-version Answers dispatch the unchanged
production choices; stale versions fail closed. The existing opaque
`AbilityLabel` extension boundary still preserves all costs, criteria,
windows, targeting, and engine messages without giving clients authority to
interpret them.

Native clients may use only the governed `SingleAction` semantic
`Investigate`, `Fight`, or `Evade` to choose a title and icon. They submit the
unchanged source index with the current question version. The backend alone
chooses legal targets, pays action costs, opens player windows, runs skill
tests, determines success, deals damage, exhausts enemies, and performs every
other rules effect. Other action semantics remain update-required data rather
than being guessed from nearby fields.

#### Basic Engage action

Revision `0.1.37` adds
`question-player-window-engage-action.json`, generated by creating the same
real Ghoul Minion engaged with Roland, running the production `EnemyEvaded`
flow, and then enumerating the ordinary production `PlayerWindow`. The enemy
runner exhausts and disengages the enemy; the resulting authoritative menu
retains Fight at source index `4` and exposes Engage at source index `5`.

The Engage entry is a real `AbilityLabel` bound to card code `c01160`, the
authoritative `EnemySource`, production ability index `102`, and action
semantic `SingleAction(Engage)`. Current-version Answers dispatch the unchanged
source choice; stale versions fail closed. Native clients may use the governed
semantic only for presentation and submit the source index with the current
question version. The backend alone decides whether the enemy can be engaged,
pays the action cost, moves it to the investigator's threat area, opens
windows, and performs every other rules effect.

#### First post-Engage round transition

Revision `0.1.38` governs the first deterministic end-of-round and Mythos
transition after Engage with four production-generated prompts:

- `question-round-end-forced-ability.json`
  (`e52ee8942cce5602a7ae68a7f8cfd98ad77b7970893ab5216413170bc0b7de44`)
  is question version 24. Its sole `AbilityLabel` is the real Dissonant
  Voices (`c01165`) forced ability at source index 0, with ability index 1,
  `RoundEnds/When`, the canonical treachery source/requestor, and the outer
  `AtEndOfRound/When` window. Answering it discards that treachery from the
  investigator's threat area.
- `question-agenda-advance.json`
  (`4d665ec3dfd3caf6dce7952ae289cae90310ca600601e3a7dbcdbb1a18727bd7`)
  is question version 25. Its sole source-index-0 `TargetLabel` advances
  agenda `c01105` with `AgendaAdvancedWithDoom`.
- `question-agenda-consequence.json`
  (`15d2e7587a32b1ab4ba4c7bb2de77645c05b3e83c03595448d49e3540749689e`)
  is question version 26. Its source indices 0 and 1 are the exact localized
  horror and random-discard scenario labels emitted by Agenda 1's reverse
  side.
- `question-agenda-horror-assignment.json`
  (`5c3ece0a8cebbd8c2a883c89bb9db753427b198fc4f5f2c9d834b036f8fe4c9`)
  is the horror branch's question version 27. It is the exact agenda-sourced
  two-horror assignment question, including the closed component, assignment,
  remaining-amount, and candidate shapes.

Current-version Answers dispatch each unchanged source index and stale versions
fail closed. Backend tests prove the treachery discard, agenda advancement,
random hand discard, and exactly two assigned horror through normal Haskell
handlers. The discard branch reaches the already-governed encounter draw at
question version 27; the horror branch reaches the same prompt at version 28,
with canonical hash
`f47283f0c5a1537cbee8cdcb7b2b5faef740a5fa44ec8a8341d92a64e10594f1`.
The Haskell constructors additionally bind repeated dynamic IDs that JSON
Schema cannot express as equality constraints.

The four schema roots are deliberately isolated rather than widening generic
...
`question-roland-defeat-reaction.json`, generated by the ordinary production
Fight flow against a real Swarm of Rats. The deterministic fixture selects the
enemy's basic Fight ability, starts the skill test, resolves a zero chaos token,
applies the successful result, defeats the enemy, and stops at Roland Banks's
actual optional reaction window.

The resulting `WindowChooseOne` has exactly two choices in authoritative order:
source index `0` is Roland's `AbilityLabel`, and source index `1` is the
existing `SkipTriggersButton`. The reaction is bound to investigator/card code
`c01001`, ability index `1`, `ReactionAbility`, `Free`, no actions, and
`PlayerLimit [PerRound, 1]`. Its criteria require Roland to be at a location
with a clue he can discover, and both declared ability windows remain the exact
`IfEnemyDefeated After You ByAny AnyEnemy` matcher.

The actual window records `IfEnemyDefeated` after damage from the defeated
enemy's basic Fight ability (index `100`). Its enemy UUID is dynamic, so the
closed JSON Schema constrains both occurrences to canonical lowercase UUIDs
while the Haskell fixture assertion binds both to the same generated enemy.
The published fixture preserves replay question version `32` and canonical
prompt digest
`7ff7e00af7be0a2b933ed1a817e7e2a54d3eaa5ae2bdce17f73f7153e59f23a9`.

Current-version answers dispatch the unchanged source indices, while stale
versions fail closed. Production execution tests prove that choosing the
reaction discovers exactly one clue and choosing Skip discovers none. Native
clients may present the governed semantic and submit the selected source index
with the current `questionVersion`; the backend alone validates criteria,
tracks the per-round limit, identifies the defeated enemy, and applies the
clue discovery. Sixty isolated single-mutation negatives reject drift in the
root/order, Roland identity, ability type/cost/actions, criteria, limit,
sources, matcher timing, defeat window, enemy UUID shape, empty message arrays,
skip control, fixed flags, and closed-object boundaries.

#### Cover Up clue-replacement reaction

Revision `0.1.40` adds `question-cover-up-reaction.json`, generated by placing
Roland Banks's real Cover Up weakness in his threat area, running the
production Fight flow through the governed Roland reaction, and stopping at
the resulting clue-discovery replacement window.

The `WindowChooseOne` again preserves authoritative order: source index `0` is
Cover Up's `AbilityLabel`, and source index `1` is `SkipTriggersButton`. The
ability is bound to card code `c01007`, ability index `1`, matching dynamic
`TreacherySource` source/requestor values, `ReactionAbility`, `Free`,
`AndActions []`, and `PlayerLimit [PerWindow, 1]`. Its criteria require the
treachery to be at Roland's location with at least one clue, and both declared
ability matchers remain the exact
`WouldDiscoverClues When You (LocationWithInvestigator You) (>= Static 1)`
shape.

The actual window records Roland `c01001`, canonical dynamic location and skill
test UUIDs, Roland's `AbilitySource` index `1`, and a one-clue discovery. The
published fixture preserves replay question version `33` and canonical prompt
digest
`8857d15cd056ac0e4d8556676dfc5746c5f9addaa2054d0b35c245610dbf71b9`.

Current-version answers dispatch the exact source indices, while stale versions
fail closed. Production execution tests prove that choosing Cover Up prevents
the pending discovery and removes one clue from Cover Up, while choosing Skip
moves one clue from the location to Roland and leaves Cover Up unchanged. The
backend alone evaluates the replacement effect and mutates all three clue
counts. Eighty-eight isolated single-mutation negatives reject drift in
ordering, identities, criteria, limits, both declared matchers, the actual
...
same source space. Each descriptor carries the original zero-based
`sourceIndex`; clients render the semantic metadata but return that unchanged
index with the current question version. Source index `12` is the Gathering
act objective: Roland Banks (`c01001`) selects objective ability index `999`
on act `c01108`, paying two clues per player from anywhere.

Paying that cost advances the server to Q35 rather than moving or resolving
the act directly. The raw Q35 `ChooseOne` contains exactly one
`TargetLabel (ActTarget "c01108")` choice whose message is
`AdvanceAct "c01108" (ActSource "c01108") AdvancedWithClues`. Its presentation
binds protocol version `1`, question version `35`, and source index `0` to the
render-only `advanceAct` semantic for act `c01108`. It deliberately carries no
second cost or objective ability metadata: those belonged to Q34 and have
already been accepted by the authoritative backend.

Contract validation pairs both semantic fixtures with their corresponding raw
...
both Q36 destinations and both Q37/Q38 branches, while twelve isolated
...
wrong movement destinations, swapped forced card codes, and wrong assignment
investigators. The backend alone moves the investigator, resolves each forced
ability, and assigns damage or horror.

#### Mythos encounter-deck draw

Revision `0.1.30` adds `question-encounter-deck-draw.json`, generated by
running the real `Begin MythosPhase` queue on a fresh clone of the same
deterministic post-setup board. Production advances through doom placement
and the agenda threshold check to
`MythosPhaseStep EachInvestigatorDrawsEncounterCardStep`; its
`ForInvestigator iid AllDrawEncounterCard` handler creates the captured
`ChooseOne [TargetLabel EncounterDeckTarget [DrawCards iid draw]]`.
...
`GameSource`, and `UnresolvedCardDraw` are closed tag-only objects, and all
twelve card-draw fields are required. Amount `1`, action `false`,
`StandardCardDraw`, `DrawFromTop`, empty rules/already-drawn arrays, and
explicitly null `cardDrawAndThen`/`cardDrawDiscard`/`cardDrawTarget` are the
supported values. Non-null continuations, matchers and targets, other draw
states/decks/sources, and modified draws remain unsupported, not guessed.
...
- **Disabled preserves the locale-specific legacy shape, not the legacy
  ...
  the monotonic bump is required for the bundle in any case. Clients compare
  the three numeric revision components and ignore unknown identifiers, so a
  client built against `0.1.22` is unaffected. `manifest.json`'s
  `legacyCompatibilityChecks` pins that exact baseline body, and both
  `contracts:fixtures` and the backend spec compare the real response against
  it with known additive capabilities normalized away.
...
Positive `fixtures` entries support only `path` and `schema`; they are always
validated against the registered schema root. `schemaBranch` is not read for
positive fixtures, so adding it there would not select a `$defs` branch.
