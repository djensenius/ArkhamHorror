...
SEALED_NODE_LAUNCHER = "scripts/locale-catalog/sealed-node-launcher.mjs"
...
def sealed_node_argv(frontend: Path, entry: str, arguments: list[str]) -> list[str]:
    ...
    return [str((frontend / SEALED_NODE_LAUNCHER).resolve()), entry, *arguments]
...
def run_generator(
    ...
    *,
    ...
    result = subprocess.run(
        [node, *sealed_node_argv(frontend.resolve(), "generate.mjs", list(args))],
...
def validate_deployment_wiring(manifest: dict) -> None:
    ...
    require(
        "sealed-node-launcher.mjs generate.mjs --check" in package["scripts"]["prebuild"]
