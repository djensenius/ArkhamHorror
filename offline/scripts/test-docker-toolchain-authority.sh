...
if ! awk '
  ...
  /docker-runtime-authority.sh node/ {checks[++check_count]=NR}
  /generator-launcher\.mjs generate\.mjs/ {generator=NR}
  /npm-cli\.js run build/ {build=NR}
  /generator-launcher\.mjs verify-dist\.mjs/ {publish=NR}
  END {
    valid = ci && check_count == 3
    valid = valid && ci < checks[1] && checks[1] < generator
    valid = valid && generator < checks[2] && checks[2] < build
    valid = valid && build < checks[3] && checks[3] < publish
    exit !valid
  }
' "${REPO_ROOT}/Dockerfile"; then
  fail "Dockerfile does not reverify Node/npm after ci and before all three execution boundaries"
fi
