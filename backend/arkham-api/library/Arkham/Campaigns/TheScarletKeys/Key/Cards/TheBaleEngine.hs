  ...
  runMessage msg k@(TheBaleEngine attrs) = runQueueT $ case msg of
    ...
    HandleTargetChoice iid (isSource attrs -> True) (AssetTarget aid) -> do
      ...
      chooseOrRunOneM iid $ withI18n do
        when canAmmo $ labeled "$token.Ammo" $ addUses attrs aid Ammo 1
        when canCharge $ labeled' "charge" $ addUses attrs aid Charge 1
        when canSecret $ labeled' "secret" $ addUses attrs aid Secret 1
        when canSupply $ labeled "$token.Supply" $ addUses attrs aid Supply 1
        when canEvidence $ labeled "$token.Evidence" $ addUses attrs aid Evidence 1
      pure k
